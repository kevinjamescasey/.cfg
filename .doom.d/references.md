...
## How config is found
OS desktop manager or shell launches installed port of GNU Emacs that looks for config in certain locations (~/.emacs.d/init.el, ~/.config/emacs/init.el).
It finds Chemacs config in one of those locations (~/.emacs.d/init.el).
...
If we remove Chemacs from ~/.emacs.d , the Doom config should be found since it is in ~/.config/emacs. 

## Emacs ports
GNU Emacs that runs in CLI terminal can be installed with this command.
```
brew install emacs 
```

It is not really compatible with any of my Doom config.

There are newer ports in the railwaycat/emacsmacport/emacs-mac cask, but they are experimental.
